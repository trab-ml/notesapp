...
export const useNotes = (userId?: string) => {
    ...
    const assertUserConnected = useCallback((message: string) => {
        if (!userId) throw new Error(message);
    }, [userId]);

    const assertOnline = useCallback((message: string) => {
    ...
    const addNewNote = useCallback(async (noteData: Omit<INote, "id" | "createdAt" | "updatedAt" | "sharedWith">) => {
        assertUserConnected("Veuillez vous connecter pour ajouter une note");
        await notesService.addNote(noteData);
    }, [assertUserConnected]);

    const updateExistingNote = useCallback(async (noteId: string, updates: Partial<INote>) => {
        assertUserConnected("Vous devez être connecté pour modifier une note");
